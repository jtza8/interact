...
(def-test-method test-order-top-bottom ((test container-test))
  (let ((widget (make-instance 'dummy-widget))
    ...
    (assert-eql widget (aref (widgets container) 2) "C")
    (assert-false (order-top container widget) "D")
    (assert-eql widget (aref (widgets container) 2) "E")
    (assert-true (adjustable-array-p (widgets container)) "F")
    (assert-true (order-bottom container widget) "G")
    (assert-eql widget (aref (widgets container) 0) "H")
    (assert-true (adjustable-array-p (widgets container)) "I")))
