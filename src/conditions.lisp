...
(define-condition widget-tag-error (error)
  ((fault :initarg :fault
   ...
   (widget :initarg :widget
        :initform nil))
  (:report (lambda (condition stream)
             (reason-reporter condition stream fault (tag widget)
               (:duplicate-tag "Tag ~s for widget ~s must be unique" tag widget)
               (:double-tag "IGO ~s already has tag ~s" widget tag)
