...
(def-test-method test-tagging ((test container-test))
  (let ((igo-1 (make-instance 'dummy-igo))
    ...
    (assert-condition 'igo-tag-error (tag-igo container igo-2 :igo-1))
    (assert-condition 'igo-tag-error (tag-igo container igo-1 :igo-2))
    (assert-eql igo-1 (igo-of container :igo-1))
    (tag-igo container igo-2 :igo-2)
    (assert-condition 'igo-tag-error (igo-of container :igo-3))
    (remove-tag container igo-1)
    (assert-condition 'igo-tag-error (igo-of container :igo-1))
    (assert-eql igo-2 (igo-of container :igo-2))
    (remove-tag container :igo-2)
    (assert-condition 'igo-tag-error (igo-of container :igo-2))))
